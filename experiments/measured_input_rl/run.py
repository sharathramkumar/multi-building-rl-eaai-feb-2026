...
from MultiBuildingRLEnv.envs.rl_envs_dev.ClusterTrainingEnvWithGlobal import (
    ClusterTrainingEnvWithGlobalCallback,
)
from MultiBuildingRLEnv.evaluation.pricing_strategy import (
    StaticPricingStrategy,
    IndividualRLPricingStrategyWithGlobal,
)
...
cluster_indices = np.array(
    [0] * len(cons_models)
)  # Generated using "clustering_script.py" features
...
def get_norm_env(cm):
    return SingleConsTrainingEnvWithGlobal(
...
def get_env(cluster_idx):
    ...
    return ClusterTrainingEnvWithGlobal(
...
for net_size in actor_network_sizes:
    for seed in seeds:
        ...
        for cluster in uniq_clusters:
            ...
            model.learn(
                int(n_train_steps),
                callback=ClusterTrainingEnvWithGlobalCallback(),
            )
            model_save_path = os.path.join(
                os.getcwd(), "models", expt_tag + f"_{seed}.mdl"
            )
            model.save(model_save_path)
            logging.info(f"Done! Saved model to {model_save_path}")
            rl_models_cluster[cluster] = model

        # At this stage, all the models for this seed are ready
        logging.info(f"Testing the agent in the real system..")
        rl_models = [rl_models_cluster[c] for c in cluster_indices]
        t_rl_ps = IndividualRLPricingStrategyWithGlobal(rl_models, rl_norm_envs)
